...
@Component({
  selector: 'app-dem-naissance',
  templateUrl: './dem-naissance.component.html',
  styleUrls: ['./dem-naissance.component.css']
})
export class DemNaissanceComponent implements OnInit {

  constructor(public service: DemandeService, private toastr: ToastrManager
    ,public serv_livraison: LivraisonService)  { }
...
invalideDemande(form: NgForm){
  ...
  this.service.getDemande(form.value.id).subscribe((data: Demande)=>{
    ...
    this.d = data;
    var dat= new Date("2020-10-10T" + data.idNaissance.heurenaiss + ".00");
    this.d.idNaissance.heurenaiss= dat;

  this.d.commentaire = form.value.commentaire;
  this.d.etat = false;
 
  this.service.putDemande(this.d).subscribe((data1: Demande)=>{
   
    this.serv_livraison.getLivraisonBytype(1, data1.id, "demande").subscribe((data2: Livraison)=>{
    console.log(data2);

    if(data2.boiteEmail != ""){
    
    this.data = new Email();
  this.data.email= data2.boiteEmail;
  this.data.objet= "Invalidation acte de naissance";
  this.descript = "Nous venons par la presente, vous annoncer que votre demande d'acte de naissance enregistrée sous le numéro de registre : " + this.d.numero + "  a ete rejete <br> Veuillez trouver ci-apres les motifs du rejet: <br> " + this.d.commentaire ;
  this.data.body= "<h3>Bonjour cher "+ this.d.civilite + " , " + this.d.nom + "!</h3><br><h5>" + this.descript + "</h5> <br> Cordialement!";
 this.data.attach= "";
 ...
 this.service.sendEmail(this.data).subscribe((res)=>{
  ...
  this.toastr.successToastr('Statut invalidée réussi!', ' Demande acte');
  }
